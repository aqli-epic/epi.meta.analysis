...
## Inclusion/Exclusion Criteria for studies
  ...
  * In cases where the same cohort is studied by different research groups at different point in time/using different methods: we have included and counted all of those studies as separate unique studies in the analysis below.
...
mean_pm2.5_country_wise_graph <- epi %>% 
  ...
  labs(x = expression(paste("Mean PM2.5 concentration (", mu, "g", "/", m^3, ")")), 
       caption = "*This graph 'only' takes into account the PM2.5 specific studies. For multi-country (pooled) studies, it averages the mean PM2.5 values, across all countries.") +
  theme_tufte()
...
# total number times USA and Canada have been included in a study
num_times_usa_canada_included <- epi_country_count %>% 
...
```
  ...
  * Of the total number of times any given country is included in any given study, USA and Canada together have been included `r num_times_usa_canada_included` times (`r round((num_times_usa_canada_included/sum(epi_country_count$n, na.rm = TRUE))*100, 1)` percent of total). 
  
  * USA, Canada and Europe combined have been included `r num_times_usa_canada_europe_included` times (`r round((num_times_usa_canada_europe_included/sum(epi_country_count$n, na.rm = TRUE))*100, 1)` percent of total). Other countries that have been included at some point in a AQ epi study are: `r areas_other_than_usa_canada_europe`.
  
  * Of the total number of times, a given continent has been included in any given study, `r continent_wise_study_summary$continent[1]` dominates the rest of the continents and has been included `r continent_wise_study_summary$prop_studies[1]` percent of times. Closely following `r continent_wise_study_summary$continent[1]`, are `r continent_wise_study_summary$continent[2]` and `r continent_wise_study_summary$continent[3]` which have been included in a study `r continent_wise_study_summary$prop_studies[2]` and `r continent_wise_study_summary$prop_studies[3]` percent of times respectively. `r continent_wise_study_summary$continent[4]` has only been included `r continent_wise_study_summary$prop_studies[4]` percent of times, while `r continent_wise_study_summary$continent[5]` and `r continent_wise_study_summary$continent[6]` have been included the least number of studies (`r continent_wise_study_summary$prop_studies[5]` and `r continent_wise_study_summary$prop_studies[6]` percent respectively).
...
* In total, 5 “really” long term studies (> 25 years) have been performed. That is, 4.9 percent of the total number of studies.

* All of the following continents have seen at least 1 really long term study: `r epi_long_term_studies_countries %>% filter(n >= 1) %>% select(continent) %>% unlist() %>% as.vector()`. While, only `r epi_long_term_studies_countries %>% filter(n > 1) %>% select(continent) %>% unlist() %>% as.vector()` have seen more than 1 really long term study.
