...
ui <- shinydashboard::dashboardPage(
  ...
  shinydashboard::dashboardSidebar(
    ...
    shinydashboard::sidebarMenu(
      ...
      shinydashboard::menuItem(tabName = "underlying_data_table", text = "Underlying Data", icon = icon("database")),
  ...
  shinydashboard::dashboardBody(
    shinydashboard::tabItems(
      ...
      shinydashboard::tabItem("underlying_data_table",
                              shiny::fluidRow(
                                shinycssloaders::withSpinner(DT::dataTableOutput("underlying_data_table_1")))
                              ),
...
server <- function(input, output, session) {
  ...
  output$age_ll_ul_graph <- shiny::renderPlot({
    ...
    if(input$plot_type_fig6 == "Histogram"){
      epi_age_dist %>%
        ...
        ggplot2::scale_x_continuous(breaks = seq(0, 130, 5)) +
  ...
  output$underlying_data_table_1 <-  DT::renderDT(
     return(DT::datatable(epi, options = list(autoWidth = TRUE, scrollX = TRUE)))
